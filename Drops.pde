...
draw = function() {
    ...
    for (var i = 0; i < xPositions.length; i++) {
        ...
        if(yPositions[i]>250 && yPositions[i]<300 && xPositions[i]<BlobbX+25 && xPositions[i]>BlobbX-25){
            println("GAME OVER");
            textSize(50);
            text("Poäng" + poäng, 131,130,128,125);
            draw = 1;
        }
    }
    ...
    poäng++;
    ökaladdningshastighet++;
    if (ökaladdningshastighet>1000){
        ökaladdningshastighet = 0;
        laddahastihet++;
        println("level up");
    }
    if(ladda<0){
